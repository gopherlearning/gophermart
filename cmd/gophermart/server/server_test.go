...
func TestSpec(t *testing.T) {
	Convey("Тестируем сервер", t, FailureContinues, func() {
		...
		Convey("Тестируем через http", func() {
			...
			wgLocal.Add(4)
			// Convey("Собственные тесты", FailureContinues,
			func() {
				defer func() {
					loger.Error("Собственные тесты")
					wgLocal.Done()
				}()
				jar, err := cookiejar.New(&cookiejar.Options{PublicSuffixList: publicsuffix.List})
				So(err, ShouldBeNil)
			...
			// Convey("TestGophermart/TestEndToEnd", FailureContinues,
			func() {
				defer func() {
			...
			// Convey("TestGophermart/TestUserAuth", FailureContinues,
			func() {
				defer func() {
				...
				for _, f := range requestsTestUserAuth {
			...
			// Convey("TestGophermart/TestUserOrders", FailureContinues,
			func() {
				if r := recover(); r != nil {
